...
<body>
<div class="container">
<div class="row" style="float:left;"><a href="${pageContext.request.contextPath}/tbloginfo/showHotBlog" style="color: blue;text-decoration: underline;">点我?返回首页^_^</a>
对不起，因为个人时间和精力有限，最近急于找工作，所以有些算法交互没认真考虑，逻辑不严谨，中文出现乱码,等有时间修改部分bug，请谅解!谢谢
</div>
<br/><br/>
<!-- Split button -->
...
<div class="btn-group">
  ...
  <c:forEach items="${algotithms.object}" var="algoList">	
    <li><a href="javascript:getAlgoByIndex('${algoList.funcIndex}')" class="am-btn-info-algo">${algoList.algotithmsTitle}</a></li>
    </c:forEach>
  </ul>
</div>
</c:forEach>
...
<div class="row">
  ...
  <br/>
  <div class="col-md-7" style="margin-left: 15%;"><input type="text" class="form-control" placeholder="Text input" id="yourInput">
  <br/>
 ...
 */
 function getAlgoByIndex(id){
		$.ajax({
			cache: false,
			type: "POST",
			url: '${pageContext.request.contextPath}/algorithms/showAlgorithms ',  
			data: {
				"funcIndex" : id
			},
			 dataType:"json",
 ...
 $(".btn-alog-submiit").click(function(){
	 ...
	 var yourInput=$("#yourInput").val();
		...
		$.ajax({
			cache: false,
			type: "POST",
			url: 'http://111.230.246.33:8080/AlgorithmsConsume/algorithms/'+funcName,  
			data: {
				"yourInput" : yourInput
			},
			 dataType:"json",
